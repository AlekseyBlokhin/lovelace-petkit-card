function b(o){let t=Math.round(o||0);if(t<60)return`${t}s`;let e=Math.floor(t/60),i=t%60;return`${e}m${i.toString().padStart(2,"0")}s`}function O(o){let t=Math.round(o||0),e=Math.floor(t/60),i=t%60;return`${e.toString().padStart(2,"0")}'${i.toString().padStart(2,"0")}"`}function V(o){let t=Math.floor(o);return t<1?"under 1h":t<48?`${t}h`:`${Math.floor(t/24)}d`}function y(o){return String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function A(o,t=new Date){let e=new Date(t.getFullYear(),t.getMonth(),t.getDate());e.setDate(e.getDate()+o);let i=new Date(e);return i.setDate(i.getDate()+1),{start:e,end:i}}function F(o,t=new Date){if(o===0)return"Today";if(o===-1)return"Yesterday";let{start:e}=A(o,t);return e.toLocaleDateString(void 0,{weekday:"short",month:"short",day:"numeric"})}function k(o){let t=new Date(o);return`${t.getFullYear()}-${t.getMonth()}-${t.getDate()}`}function T({startTime:o,endTime:t,entityIds:e,includeStartTimeState:i=!1}){let n=o instanceof Date?o.toISOString():o,s=t instanceof Date?t.toISOString():t;return{type:"history/history_during_period",start_time:n,end_time:s,entity_ids:e,minimal_response:!1,no_attributes:!0,include_start_time_state:i}}function xt(o){if(!o)return null;let t=o.s??o.state,e=parseFloat(t),i=o.lu?o.lu*1e3:o.last_changed?Date.parse(o.last_changed):null;return!Number.isFinite(e)||!i||Number.isNaN(i)?null:{value:e,ts:i}}function z(o,{minDelta:t=0,maxDelta:e=1/0}={}){if(!Array.isArray(o))return[];let i=o.map(xt).filter(Boolean).sort((s,a)=>s.ts-a.ts),n=[];for(let s=1;s<i.length;s++){let a=i[s].value-i[s-1].value;if(a<=t||a>=e)continue;let r=i[s+1];r&&r.value===i[s-1].value||n.push({value:a,ts:i[s].ts})}return n}function U(o,t){if(!Array.isArray(o))return[];let e=new Set(t),i=[];for(let n of o){let s=n.s??n.state;if(!e.has(s))continue;let a=n.lu?n.lu*1e3:n.last_changed?Date.parse(n.last_changed):null;!a||Number.isNaN(a)||i.push({cat:s,ts:a})}return i.sort((n,s)=>n.ts-s.ts),i}var bt=15e3;function P(o,t,{toleranceMs:e=bt}={}){let i=0,n=null;return o.map(s=>{for(;i<t.length&&t[i].ts<=s.ts+e;)n=t[i].cat,i++;return{...s,cat:n}})}var wt=[10,15,30,60,120,180,300,600,900,1800,3600];function W(o){for(let t of wt)if(o/t<=5)return t;return Math.ceil(o/5/60)*60}function j({dayStart:o,niceMax:t,width:e,height:i,padding:n}){let{left:s,right:a,top:r,bottom:l}=n,p=e-s-a,h=i-r-l,d=o.getTime();return{xFor:g=>{let L=(g-d)/36e5;return s+L/24*p},yFor:g=>t?i-l-g/t*h:i-l}}function q({niceMax:o,yStep:t,width:e,height:i,padding:n}){let{left:s,right:a,top:r,bottom:l}=n,p=e-s-a,h=i-r-l,d=[4,8,12,16,20].map(u=>({hour:u,x:s+u/24*p,label:`${u.toString().padStart(2,"0")}:00`})),f=[];if(t>0)for(let u=0;u<=o;u+=t){let g=o?i-l-u/o*h:i-l;f.push({value:u,y:g,label:O(u)})}return{vertical:d,horizontal:f}}function G(o,{dayKeyFn:t}){let e={};for(let i of o||[]){let n=t(i.ts);e[n]||(e[n]={count:0,total:0}),e[n].count+=1,e[n].total+=i.value}return e}function K(o,t){let e=Object.keys(o).filter(h=>h!==t).sort(),i=e.slice(-3),n=e.slice(-7),s=(h,d)=>h.length?h.reduce((f,u)=>f+o[u][d],0)/h.length:null,a=h=>{if(!h.length)return null;let d=h.reduce((u,g)=>u+o[g].total,0),f=h.reduce((u,g)=>u+o[g].count,0);return f>0?d/f:null},r=o[t],l=r?r.count:0,p=r?r.total:0;return{todayCount:l,todayTotal:p,todayAvgDuration:l>0?p/l:null,avg3dVisits:s(i,"count"),avg3dTotal:s(i,"total"),avg3dDuration:a(i),avg7dVisits:s(n,"count"),avg7dTotal:s(n,"total"),avg7dDuration:a(n),daysOfHistory:e.length}}function X({lastVisitTs:o,now:t,thresholdHours:e}){let i=t instanceof Date?t.getTime():t;if(o==null)return{alerting:!0,hoursSince:null};let n=(i-o)/36e5;return{alerting:n>=e,hoursSince:n}}function Y(o,t){let e=t;o.value_map&&t in o.value_map?e=o.value_map[t]:t!==null&&o.unit&&(e=`${t}${o.unit}`),e==null&&(e="\u2014");let i=parseFloat(t),n=!1;return o.warn_below!==void 0&&Number.isFinite(i)&&(n=i<o.warn_below),o.warn_above!==void 0&&Number.isFinite(i)&&(n=n||i>o.warn_above),o.warn_state!==void 0&&(n=n||t===o.warn_state),{display:e,warn:n}}function J(o,t,e){return!t||!o||!o.states||!o.states[t]?e:o.states[t].state}function Q(o,t){let e=new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:t}});o.dispatchEvent(e)}function R(o,t,e,i){o.callService(t,e,i)}function E(o,t){t&&R(o,"button","press",{entity_id:t})}var Z=`
...
`;var tt={maintenance_mode:"Maintenance mode",manual_odor_completed:"Manual odor removal done",auto_cleaning_completed:"Auto cleaning done",no_events_yet:null,unavailable:null,unknown:null},et=/ used the litter box$/,it="PETKIT PURAMAX",nt=60,ot=8,st=1800,at=600,rt=240,ct={left:46,right:10,top:10,bottom:28};var $=class extends HTMLElement{static getConfigElement(){return document.createElement("petkit-puramax-card-editor")}static getStubConfig(){return{type:"custom:petkit-puramax-card",title:"PETKIT PURAMAX",device_entities:{error:"sensor.example_petkit_error",last_event:"sensor.example_petkit_last_event",state:"sensor.example_petkit_state",total_use:"sensor.example_petkit_total_use"},cats:[{name:"Example Cat",color:"#4fc3f7"}]}}setConfig(t){if(!t)throw new Error("petkit-puramax-card: config is required");if(!t.device_entities)throw new Error('petkit-puramax-card: "device_entities" is required in config');if(!t.device_entities.total_use)throw new Error('petkit-puramax-card: "device_entities.total_use" is required in config');if(!t.cats)throw new Error('petkit-puramax-card: "cats" is required in config');if(!Array.isArray(t.cats)||t.cats.length<1)throw new Error('petkit-puramax-card: "cats" must be a non-empty array');if(t.cats.length>1&&!t.device_entities.last_used_by)throw new Error('petkit-puramax-card: "device_entities.last_used_by" is required in config when more than one cat is configured');t.cats.forEach((e,i)=>{if(!e||!e.name)throw new Error(`petkit-puramax-card: cats[${i}].name is required`);if(!e.color)throw new Error(`petkit-puramax-card: cats[${i}].color is required`)}),this._config=t,this._dayOffset=0,this._analytics=null,this._chartData=null,this._loadingChart=!1,this._loadingAnalytics=!1,this.shadowRoot||this.attachShadow({mode:"open"}),this._render()}set hass(t){let e=this._hass;if(this._hass=t,!this._built){this._built=!0,this._render(),this._loadDay(),this._loadAnalytics(),this._startNoVisitTimer();return}this._updateLiveValues(),this._maybeRefreshOnNewVisit(e)}get hass(){return this._hass}getCardSize(){return 14}connectedCallback(){this._hass&&!this._built&&(this._built=!0,this._render(),this._loadDay(),this._loadAnalytics(),this._startNoVisitTimer())}disconnectedCallback(){this._noVisitTimer&&(clearInterval(this._noVisitTimer),this._noVisitTimer=null)}_startNoVisitTimer(){this._noVisitTimer||(this._noVisitTimer=setInterval(()=>this._checkNoVisitAlerts(),300*1e3))}_maybeRefreshOnNewVisit(t){if(!t)return;let e=this._config.device_entities.total_use,i=t.states[e],n=this._hass.states[e];n&&(!i||i.last_changed!==n.last_changed)&&(this._dayOffset===0&&this._loadDay(),this._loadAnalytics())}_s(t,e){return J(this._hass,t,e)}async _fetchVisits({start:t,end:e}){let i=this._config,n=T({startTime:t,endTime:e,entityIds:[i.device_entities.total_use]}),s=[this._hass.callWS(n)];if(i.cats.length>1){let d=T({startTime:t,endTime:e,entityIds:[i.device_entities.last_used_by],includeStartTimeState:!0});s.push(this._hass.callWS(d))}let a={},r={};try{let[d,f]=await Promise.all(s);a=d||{},r=f||{}}catch{}let l=z(a[i.device_entities.total_use],{minDelta:0,maxDelta:st}),p;if(i.cats.length>1){let d=i.cats.map(u=>u.name),f=U(r[i.device_entities.last_used_by],d);p=P(l,f)}else p=l.map(d=>({...d,cat:i.cats[0].name}));let h=new Map(i.cats.map(d=>[d.name,d]));return p.map(d=>({cat:h.get(d.cat)||null,duration:d.value,ts:d.ts}))}async _loadDay(){if(!this._hass)return;this._loadingChart=!0,this._renderChartArea();let t=this._config,{start:e,end:i}=A(this._dayOffset),n=[],s=[];try{let[a,r]=await Promise.all([this._fetchVisits({start:e,end:i}),t.device_entities.last_event?this._hass.callWS(T({startTime:e,endTime:i,entityIds:[t.device_entities.last_event]})):Promise.resolve({})]);n=a,s=(r||{})[t.device_entities.last_event]||[]}catch{n=[],s=[]}this._chartVisits=n,this._chartEventHist=s,this._loadingChart=!1,this._renderChartArea()}async _loadAnalytics(){if(!this._hass)return;this._loadingAnalytics=!0,this._renderAnalyticsArea();let t=this._config,e=new Date,i=new Date(e.getTime()-168*3600*1e3),n=[];try{n=await this._fetchVisits({start:i,end:e})}catch{n=[]}let s=k(e.getTime()),a={};t.cats.forEach(r=>{let l=n.filter(d=>d.cat===r).map(d=>({value:d.duration,ts:d.ts})),p=G(l,{dayKeyFn:k}),h=l.length?Math.max(...l.map(d=>d.ts)):null;a[r.name]={...K(p,s),lastVisitTs:h}}),this._analytics=a,this._loadingAnalytics=!1,this._checkNoVisitAlerts(),this._renderAnalyticsArea()}_checkNoVisitAlerts(){if(!this._analytics||!this._built)return;let t=this._config,e=t.no_visit_alert_hours??ot,i=Date.now();this._notifiedCats||(this._notifiedCats=new Set),t.cats.forEach(n=>{let s=this._analytics[n.name];if(!s)return;let a=X({lastVisitTs:s.lastVisitTs,now:i,thresholdHours:e});s.noVisitAlert=a;let r=this._notifiedCats.has(n.name);a.alerting&&!r?(this._notifiedCats.add(n.name),this._sendNoVisitNotification(n,a)):!a.alerting&&r&&this._notifiedCats.delete(n.name)}),this._renderAnalyticsArea()}_sendNoVisitNotification(t,e){let i=this._config.notify_service;if(!i||!this._hass)return;let n=i.indexOf(".");if(n===-1)return;let s=i.slice(0,n),a=i.slice(n+1);if(s!=="notify"||!a)return;let r=e.hoursSince==null?`${t.name} hasn't used the litter box yet in the tracked history.`:`${t.name} hasn't used the litter box in over ${Math.floor(e.hoursSince)}h.`;R(this._hass,"notify",a,{message:r,title:"Litter box alert"})}_render(){let t=this._config;this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.getElementById("prev-day").addEventListener("click",()=>this._changeDay(-1)),this.shadowRoot.getElementById("next-day").addEventListener("click",()=>this._changeDay(1)),this._updateLiveValues(),this._renderChartArea(),this._renderAnalyticsArea()}_changeDay(t){this._dayOffset+=t,this._dayOffset>0&&(this._dayOffset=0),this._loadDay()}_updateLiveValues(){if(!this._built)return;let t=this._config,e=t.device_entities,i=this.shadowRoot.getElementById("status-row");if(i){let s=this._s(e.error,"no_error"),a=s&&s!=="no_error",r=t.info_row||[];i.innerHTML=[...r.map(l=>this._renderInfoChip(l)),a?this._chip("mdi:alert","Error",s.replace(/_/g," "),!0):""].join("")}let n=this.shadowRoot.getElementById("controls-row");if(n&&!n.dataset.bound){let s=t.controls_row||[];n.innerHTML=s.map((a,r)=>`
      ...
      </div>`}_renderChartArea(){if(!this._built)return;let t=this.shadowRoot.getElementById("day-label");t&&(t.textContent=F(this._dayOffset));let e=this.shadowRoot.getElementById("next-day");e&&(e.disabled=this._dayOffset>=0);let i=this.shadowRoot.getElementById("chart-area"),n=this.shadowRoot.getElementById("records-list"),s=this.shadowRoot.getElementById("usage-body");if(!i||this._loadingChart)return;let a=this._config,r=(this._chartVisits||[]).slice().sort((c,_)=>c.ts-_.ts),l=r.filter(c=>c.cat),p=at,h=rt,d=ct,f=Math.max(60,...l.map(c=>c.duration)),u=W(f),g=Math.ceil(f/u)*u,{start:L}=A(this._dayOffset),{xFor:lt,yFor:dt}=j({dayStart:L,niceMax:g,width:p,height:h,padding:d}),{vertical:H,horizontal:M}=q({niceMax:g,yStep:u,width:p,height:h,padding:d}),ht=H.map(c=>`<line x1="${c.x}" y1="${d.top}" x2="${c.x}" y2="${h-d.bottom}" class="grid-line-v" />`).join(""),ut=M.map(c=>`<line x1="${d.left}" y1="${c.y}" x2="${p-d.right}" y2="${c.y}" class="grid-line-h" />`).join(""),pt=(h-d.bottom)/h*100,_t=H.map(c=>`<div class="axis-label" style="left:${c.x/p*100}%;top:${pt}%">${c.label}</div>`).join(""),mt=M.map(c=>`<div class="axis-label-y" style="top:${c.y/h*100}%">${c.label}</div>`).join(""),ft=l.map((c,_)=>{let v=lt(c.ts),m=dt(c.duration),x=h-d.bottom;return`<line x1="${v}" y1="${x}" x2="${v}" y2="${m}" stroke="${c.cat.color}" stroke-width="2" />
              <circle class="visit-point" cx="${v}" cy="${m}" r="5" fill="${c.cat.color}" />
              <line class="visit-hit" data-idx="${_}" x1="${v}" y1="${x}" x2="${v}" y2="${m}" stroke="transparent" stroke-width="16" />`}).join("");i.innerHTML=`
      <div class="chart-wrap">
        <svg viewBox="0 0 ${p} ${h}" class="chart-svg">
          ${ut}
          ${ht}
          ${ft||""}
        </svg>
    ...
    `;let w=this.shadowRoot.getElementById("chart-tooltip"),gt=i.querySelector(".chart-wrap");if(i.querySelectorAll(".visit-hit").forEach(c=>{c.addEventListener("mouseenter",()=>{let _=l[parseInt(c.dataset.idx,10)],v=new Date(_.ts).toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"});w.textContent=`${v} \xB7 ${_.cat.name} \xB7 ${b(_.duration)}`;let m=c.getBoundingClientRect(),x=gt.getBoundingClientRect();w.style.left=`${m.left-x.left+m.width/2}px`,w.style.top=`${m.top-x.top}px`,w.classList.add("visible")}),c.addEventListener("mouseleave",()=>{w.classList.remove("visible")})}),s){let c={};a.cats.forEach(m=>{c[m.name]={count:0}}),l.forEach(m=>{c[m.cat.name].count+=1});let _=l.length,v=a.cats.map(m=>{let x=c[m.name];return`<span class="usage-cat"><span class="dot" style="background:${m.color}"></span>${m.name}: ${x.count}</span>`}).join("");s.innerHTML=`
        <div class="usage-row">
          <div class="stat-value">${_} time${_===1?"":"s"}</div>
          <div class="usage-cats">${v}</div>
        </div>
      `}let C=r.map(c=>({ts:c.ts,icon:c.cat?"mdi:cat":"mdi:help-circle-outline",color:c.cat?c.cat.color:"var(--secondary-text-color)",text:`${c.cat?c.cat.name:"Unknown cat"} just spent ${b(c.duration)} in the litter box`})),vt=this._chartEventHist||[],D=this._eventLabels(),B=null,I=!1;vt.forEach(c=>{let _=c.s??c.state,v=c.lu?c.lu*1e3:c.last_changed?Date.parse(c.last_changed):null;if(!_||!v)return;if(_ in D&&D[_]===null){I=!0;return}let m=I&&_===B;if(B=_,I=!1,m||et.test(_))return;let x=D[_]||_.replace(/_/g," ").replace(/\b\w/g,yt=>yt.toUpperCase());C.push({ts:v,icon:"mdi:information-outline",color:"var(--secondary-text-color)",text:x})}),C.sort((c,_)=>_.ts-c.ts),n&&(C.length===0?n.innerHTML='<div class="empty-note">No records for this day</div>':n.innerHTML=C.map(c=>`
        ...
        `).join(""))}_eventLabels(){return{...tt,...this._config.event_labels||{}}}_renderAnalyticsArea(){if(!this._built)return;let t=this.shadowRoot.getElementById("analytics-grid"),e=this.shadowRoot.getElementById("decline-banner"),i=this.shadowRoot.getElementById("no-visit-banner");if(!t||this._loadingAnalytics||!this._analytics)return;let n=this._config,s=(n.decline_threshold_pct||nt)/100,a=[];if(t.innerHTML=n.cats.map(r=>{let l=this._analytics[r.name]||{};return l.daysOfHistory>=3&&l.avg7dTotal&&new Date().getHours()>=18&&(l.todayTotal<s*l.avg7dTotal?a.push(`${r.name}'s usage today is well below their recent average \u2014 worth a check.`):l.todayTotal>(2-s)*l.avg7dTotal&&a.push(`${r.name}'s usage today is well above their recent average \u2014 worth a check.`)),`
